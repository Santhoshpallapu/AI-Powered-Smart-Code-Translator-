...
/**
 * Generate a prompt for code translation
 * @param {string} code - The user's code
 * @param {string} language - Programming language
 * @param {string} additionalContext - Additional context for translation
 * @returns {string} - Formatted prompt for translation
 */
export const generateTranslatePrompt = (code, language, additionalContext) => {
  return `
Language: ${language}
...
Please translate this code to a different programming language while maintaining the same functionality. Provide:
1. The translated code in the target language
2. Explanation of the translation approach
3. Any language-specific considerations or adaptations
4. Usage examples if applicable
5. Notes on differences between the original and translated versions
`;
};
...
export const generateRefactorPrompt = (code, language) => {
  return `
Language: ${language}
...
4. Clarifying naming and structure
5. Providing the refactored version
6. Explaining the benefits of the changes
`;
};
...
export const getPromptByType = (type, code, language, additionalContext) => {
  const typeMap = {
    translate: () => generateTranslatePrompt(code, language, additionalContext),
    ...
    refactor: () => generateRefactorPrompt(code, language),
  };
